...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_spot_full'])) {
    ...
    try {
        // --- 1. ADDRESS UPDATE (Prepared Statement) ---
        $addrStmt = $conn->prepare("UPDATE address SET country=?, province=?, district=?, street=?, houseNumber=? WHERE addressID=?");
        $addrStmt->bind_param("sssssi", $_POST['country'], $_POST['province'], $_POST['district'], $_POST['street'], $_POST['houseNumber'], $_POST['addressID']);
        ...
        // --- 3. SPOT UPDATE ---
        // We build the query dynamically to avoid the "extra comma" issue
        $status = $_POST['status'] ?? 'Available';
        
        if ($newPhotoName) {
            $spotQuery = "UPDATE spot SET name=?, price=?, type=?, status=?, detail=?, photo=? WHERE spotID=?";
            $spotStmt = $conn->prepare($spotQuery);
            $spotStmt->bind_param("sdssssi", $_POST['name'], $_POST['price'], $_POST['type'], $status, $_POST['detail'], $newPhotoName, $_POST['spotID']);
        } else {
            $spotQuery = "UPDATE spot SET name=?, price=?, type=?, status=?, detail=? WHERE spotID=?";
            $spotStmt = $conn->prepare($spotQuery);
            $spotStmt->bind_param("sdsssi", $_POST['name'], $_POST['price'], $_POST['type'], $status, $_POST['detail'], $_POST['spotID']);
        }
        ...
        // Log the actual error for the dev, but show a generic one to the user
...
<body class="bg-slateBg font-sans text-slate-900">
                ...
                <tr class="hover:bg-slate-50/30 transition-colors group">
                    ...
                    <td class="px-8 py-6">
                        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full border shadow-sm <?= $isAvail ? 'bg-green-50 border-green-100 text-green-600' : 'bg-red-50 border-red-100 text-red-600' ?>">
                            ...
                            <span class="text-[10px] font-black uppercase tracking-widest"><?= $isAvail ? 'avaliable' : 'unavaliable' ?></span>
                        </div>
                    </td>
    ...
    <div class="bg-white w-full max-w-2xl rounded-[3rem] p-10 shadow-2xl relative overflow-y-auto max-h-[90vh]">
        <button onclick="closeEditSpotModal()" class="absolute top-6 right-8 text-slate-400 hover:text-red-500 transition-colors">
       ...
       <form action="index.php" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="flex items-center gap-6 bg-slate-50 p-6 rounded-[2rem]">
                ...
                <div class="flex-1">
                    ...
                    <input type="file" name="photo" class="w-full text-xs mt-1" onchange="previewImage(this)">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <select name="type" id="es_type" class="bg-slate-50 border-none rounded-2xl p-5 font-bold">
                    <option value="Travel-Packages">Travel-Packages</option>
                    <option value="Luxury-Hotels">Luxury-Hotels</option>
                    <option value="Vehicle-Rental">Vehicle-Rental</option>
                    <option value="Room-Rentals">Room-Rentals</option>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <input type="text" name="country" id="es_detail" placeholder="Country" class="col-span-2 bg-slate-50 rounded-2xl p-4 border-none text-sm font-bold">
            </div>
           ...
           <button type="submit" name="update_spot_full" class="w-full bg-black text-white p-5 rounded-2xl font-black uppercase tracking-widest hover:bg-slate-800 transition-all">
    Save Changes
</button>
    </div>
</div>
    ...
    document.getElementById('es_country').value = s.country;
     document.getElementById('es_detail').value = s.detail;

    // Handle Status Radio Buttons
    // Converts to title case for matching "Available" or "Unavailable"
    const statusVal = s.status.charAt(0).toUpperCase() + s.status.slice(1).toLowerCase();
...
function previewImage(input) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
            document.getElementById('es_preview').src = e.target.result;
        }
        reader.readAsDataURL(input.files[0]);
    }
}
function closeEditSpotModal() { document.getElementById('editSpotModal').classList.add('hidden'); }
    </script>
</body>
</html>
